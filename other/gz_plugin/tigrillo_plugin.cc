	...
	class TigrilloPlugin : public ModelPlugin
	{
		...
		public: void OnRosMsg(const std_msgs::StringConstPtr &_msg)
		{
			...
			rapidjson::Document d;
   			...
   			position[0] = FL.GetInt();
   			position[1] = FR.GetInt();
   			position[2] = BL.GetInt();
   			position[3] = BR.GetInt();
		...
		private: void RosInit()
		{
			...
			ROS_INFO("Create ROS Node and subscribe to /tigrillo_ctrl/uart_actuators topic");
			...
			if (!ros::isInitialized())
			{
				int argc = 0;
				char **argv = NULL;
				ros::init(argc, argv, "tigrillo_rob",
					ros::init_options::NoSigintHandler);
			}

			// Create our ROS node. This acts in a similar manner to
			// the Gazebo node
			this->rosNode.reset(new ros::NodeHandle("tigrillo_rob"));

			// Create a named topic, and subscribe to it.
			ros::SubscribeOptions so =
			ros::SubscribeOptions::create<std_msgs::String>(
				"/tigrillo_ctrl/uart_actuators",
				1,
